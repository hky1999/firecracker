        ...
        match snapshot_type {
            ...
            SnapshotType::SoftDirty => {
                if accounting.is_armed() {
                    ...
                    // Ack: the delta is written (page cache), so the next
                    // window can open. Durability is either the in-process
                    // fsync (`defer_sync=false`) or the caller's fsync before
                    // committing its manifest — arming intentionally precedes
                    // it, as an uncommitted artifact is the caller's to
                    // discard.
